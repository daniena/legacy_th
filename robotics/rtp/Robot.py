        ...
        if name in ["manuf", "name", "comment"]:
            ...
            for link in self.links:
                if link.convention != self.links[0].convention:
                    print('robot has mixed D&H link conventions')
                    exit(1)
            self.__dict__['mdh'] = self.links[0].convention == Link.LINK_MDH;
            
        elif name == "tool":
            if not ishomog(value):
                print('tool must be a homogeneous transform')
                exit(1)
            self.__dict__[name] = value;
            ...
            v = arg2array(value);
            if len(v) != 3:
                print('gravity must be a 3-vector');
            self.__dict__[name] = mat(v).T
            
        elif name == "base":
            if not ishomog(value):
                print('base must be a homogeneous transform');
                exit(1)
            self.__dict__[name] = value;
